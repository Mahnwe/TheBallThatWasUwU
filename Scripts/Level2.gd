...
func _ready():
	...
	if $SaveManager.get_properties_value("Chests", "level_two_chest"):
...
func handle_player_actions_when_level_finished():
	if $Player.position.x == finish_position_x and $Player.position.y == finish_position_y and Input.is_action_just_pressed("next_level"):
		...
		$Finish/FinishUI.get_child(5).get_child(0).load(LEVEL_3_SCENE)
...
func _on_finish_player_entered():
	...
	queue.saveData()
	if queue.current_medal >= $SaveManager.get_properties_value("medals", "level_two_medal"):
		$SaveManager.save_properties_value("medals", "level_two_medal", queue.current_medal)
	$Player.get_child(0).get_child(0).get_child(0).get_child(1).instantiate(queue.file_data)
	$SaveManager.save_properties_value("levels", "is_level_two_finished", true)
	$Player.get_child(1).animation = "stay"
	$Player.set_physics_process(false)
	...
	var number_of_level_two_finished = $SaveManager.get_stats_value("Stats", "level_two_finished_number")
	$SaveManager.save_stats_value("Stats", "level_two_finished_number", number_of_level_two_finished+1)
	save_medals_stats()
...
func translate_text():
	...
	$Ability.get_child(0).get_child(0).text = translate_config.get_value("TranslationAbility", "DashAbility")
...
func _on_chest_chest_triggered():
	$SaveManager.save_properties_value("Chests", "level_two_chest", true)
