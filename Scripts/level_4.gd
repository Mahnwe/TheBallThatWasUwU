...
func _ready():
	...
	if $SaveManager.get_properties_value("Chests", "level_four_chest"):
...
func _on_finish_player_entered():
	...
	queue.saveData()
	if queue.current_medal >= $SaveManager.get_properties_value("medals", "level_four_medal"):
		$SaveManager.save_properties_value("medals", "level_four_medal", queue.current_medal)
	$Player.get_child(0).get_child(0).get_child(0).get_child(1).instantiate(queue.file_data)
	$SaveManager.save_properties_value("levels", "is_level_four_finished", true)
	...
	var number_of_level_four_finished = $SaveManager.get_stats_value("Stats", "level_four_finished_number")
	$SaveManager.save_stats_value("Stats", "level_four_finished_number", number_of_level_four_finished+1)
	save_medals_stats()
...
func _on_chest_chest_triggered():
	$SaveManager.save_properties_value("Chests", "level_four_chest", true)
